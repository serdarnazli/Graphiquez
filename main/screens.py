...
class Screens():
    def __init__(self, database):
        ...
        if sys.platform.startswith("linux") or sys.platform.startswith("darwin"):
            ...
            print(
                "The birds say you are using an unsupported operating system. This program can only run on computers with Linux, MacOS and Windows installed. If you think there is something wrong, please contact with the manufacturer!")
            time.sleep(10)
            exit()
    ...
    def log_in_screen(self):
        while True:
            ...
            if rememberMe == "NoProblem":
                willRememberWork = input(
                    f"We remembered you {user_name}! Would you like to use this account(hit just enter) or login with different account?(type 0)")
            ...
            if message == "NoProblem":
                ...
                process = input(
                    f"There is not any account with username: {user_name}. To try again please type something. To go back please type 1 ")
                if process == "1":
    ...
    def settingsScreen(self):
        while True:
            ...
            if int(process) not in range(14):
                ...
                print(
                    f"Setting is changing. '{settingList[int(process)][0]}' setting value from '{settingList[int(process)][1]} to '{tochange}'")
